(ns org.nfrac.liquidfun.testbed.tests.edge-points
  "An example of liquidfun.vec2/poly-edge-point"
  (:require [org.nfrac.liquidfun.testbed :as bed]
            [org.nfrac.liquidfun.core :as lf :refer [body! joint! fixture!]]
...
(defn setup []
  (let [world (lf/new-world)
    ...
    (doseq [jt spokes2] (lf/enable-motor! jt false))
...
(defn draw
  [state]
  (bed/draw state)
  (let [jts (::joints state)
        jt (first jts)]
    (quil/fill 255)
    (quil/text (str "Motor " (if (lf/motor-enabled? jt) "on " "off ")
                    (if (pos? (lf/motor-speed jt)) "left" "right") "\n"
                    "Keys: (m) motor, (a) left, (d) right")
               10 10)))
...
(defn my-key-press
  ...
  (let [jts (::joints state)]
    (case (:key event)
      :m (do (doseq [jt jts]
               (lf/enable-motor! jt (not (lf/motor-enabled? jt))))
             state)
      :a (do (doseq [jt jts]
