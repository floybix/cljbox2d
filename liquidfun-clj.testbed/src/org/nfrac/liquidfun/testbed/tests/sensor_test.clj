...
(defn sensor-touching-listener
  ...
  (proxy [liquidfun$b2ContactListener] []
    (BeginContact [^liquidfun$b2Contact contact]
      (let [fixt-a (.GetFixtureA contact)
        ...
        (when bod
          (swap! touching conj bod))))
   (EndContact [^liquidfun$b2Contact contact]
     (let [fixt-a (.GetFixtureA contact)
           fixt-b (.GetFixtureB contact)
           bod (cond
                (= fixt-a sensor-fixt) (lf/body-of fixt-b)
  ...
  (let [world (lf/new-world)
        ...
        ballseq (for [i (range 7)
                  ...
                  (body! world {:position [x 20]}
                         {:shape (lf/circle 1)}))
        lis (sensor-touching-listener (lf/fixture-of sens))]
